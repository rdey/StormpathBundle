                ...
                ->arrayNode('client')->isRequired()
                    ->validate()
                        ->ifTrue(function($node) {
                            return !(isset($node['api_key_file']['path']) xor (isset($node['api_key_id']) && isset($node['api_key_secret'])));
                        })
                        ->thenInvalid('You must either set api_key_file, or api_key_id and api_key_secret. You cannot set both.')
                    ->end()
                    ->validate()
                        ->ifTrue(function($node) {
                            return isset($node['api_key_id']) xor isset($node['api_key_secret']);
                        })
                        ->thenInvalid('You need to set both the api_key_id and the api_key_secret.')
                    ->end()
                    ->children()
                        ->scalarNode('api_key_id')->end()
                        ->scalarNode('api_key_secret')->end()
                        ->arrayNode('api_key_file')
                            ->beforeNormalization()
                                ->ifString()
